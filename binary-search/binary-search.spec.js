...
describe("Binary Search", () => {
  ...
  it("returns false if target value is not valid or array is empty", () => {
    expect(binarySearch([], 25)).to.equal(false);
    expect(binarySearch(test3)).to.equal(false);
    expect(binarySearch(test3, null)).to.equal(false);
    expect(binarySearch(test3, undefined)).to.equal(false);
  });
});
